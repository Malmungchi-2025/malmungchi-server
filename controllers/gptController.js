...
exports.generateQuiz = async (req, res) => {
  try {
    ...
    const prompt = `
    너는 어휘력 학습용 퀴즈 생성기야.
    사용자에게 총 7개의 문제를 생성해야 한다:
    - 4지선다형 3문제
    - O/X형 2문제
    - 단답형 2문제
    문제 순서는 랜덤이며, 난이도는 "${cfg.category}"(수준별 기준 참고)에 맞게 구성한다.
    ...
    - 반드시 한국어로 작성
    - 정치, 종교, 시사 금지
    - 문제 자체의 문장 또는 질문 문장은 짧지만 함축적·복합적 의미를 가져야 함
    - 출력은 반드시 JSON 배열만
    - 각 항목 형식:
      {
        "type": "mcq | ox | short",
        "question": "...",
        "options": [...],     // mcq일 때만 4개, 그 외 [] 가능
        "answer": "..."
      }
    ...
    - options 반드시 4개
    - 정답은 1개
    - 오답 보기들은 실제 비즈니스·학술적 맥락에서 오용될 가능성이 높은 함정 보기로 구성
    - 문맥 적합성을 판단해야 하고 단순 동의어 문제는 금지

    2) O/X형(ox)
    - 복잡한 문맥·해석을 요구하는 문장을 제시 후 O 또는 X 답하도록 구성
    - 단순 사실 확인 문제 금지
    - 직장·사회생활 맥락 기반이어야 함
    ...
    - 단순 단어 맞히기 금지
    - 다음과 같은 형태 허용:
      • '밑줄 친 단어를 상황에 맞게 바꿔 쓰세요'
      • '다음 설명을 충족하는 전문 용어를 쓰세요'
      • '주어진 문장에서 빈칸에 들어갈 정확한 개념어를 쓰세요'
    - 정답은 반드시 하나로 명확해야 하며 모호성 제거
    ...
    1) [취업준비]
    - 실제 비즈니스 상황(면접·자소서·보고서·협상·회의)
    - 미묘한 비즈니스 표현을 구분하는 문제
    - 예: 지향/지양, 제고/고취, 피력/역설, 함의/시사점, 간극/격차

    2) [기초]
    - 일상적이지만 헷갈리기 쉬운 기본 어휘
    - 동음/다의어, 문맥별 의미 차이
    - 예: ~로서/로써, 개발/계발, 금세/금새, 이따금/이따가

    3) [활용]
    - 실무 보고서·공지 기반
    - 전문 용어 의미 구분
    - 예: 고지/공지, 소상히/상세히, 심사숙고/재고, 대두되다/부각되다
    ...
    [
      ...
      { "type": "ox", "question": "...", "options": [], "answer": "O" },
      { "type": "short", "question": "...", "options": [], "answer": "..." },
      ...
    ]

    JSON 배열 외의 그 어떤 텍스트도 출력하지 마라.
    `.trim();
