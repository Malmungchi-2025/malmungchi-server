...
exports.generateQuote = async (req, res) => {
  try {
    ...
    const COMMON_RULES = `
      ...
      - 따옴표(" ' “ ” 등) 절대 금지.
      - 질문문 금지.
      - 독자에게 말 거는 표현 금지(~하세요, ~할까요 등).
      - 제목 금지.
      - 코드블록 금지.
      - 메타설명, 사족, 서두 멘트 금지.
      - 특정 기관 이름·뉴스매체 이름·통계 수치 언급 금지.
      - 사실 기반처럼 자연스럽게 구성하되 구체적 출처는 쓰지 말 것.
      `;

    // ────────── 프롬프트 개선판 ──────────
    const levelPrompts = {
      1: `
    20대 사회초년생 문해력 강화용 글을 작성한다.
    주제는 직장, 일상, 관계, 감정, 습관 중 1개로 한다.
    문장은 짧되, 의미 밀도를 높여 단순한 감정 나열이 아닌 명확한 사건·상황 묘사를 포함한다.
    초급 난이도지만 지나치게 유치하거나 어린아이가 쓴 글처럼 구성하지 않는다.
    어휘: 금일, 작성, 절차, 과정, 변동 등 기본 행정·생활 어휘 중 3개 이상 반드시 포함한다.
    문장 구성: 단순문 중심이지만 동일한 어휘 반복을 피하고 연결을 자연스럽게 유지한다.
    서술 방식: 담백하되 정보량이 있는 형태.
    분량: 480~520자.
    ${COMMON_RULES}
      `,
    
      2: `
    20대 사회초년생을 위한 실무형 문해력 글을 작성한다.
    문체는 안내문·보고서·실무 지침서 스타일을 참고해 단정하고 구조적인 문장으로 구성한다.
    하루의 사건이나 특정 상황을 기반으로 "현황 설명 → 과정 또는 이유 서술 → 간단한 시사점" 흐름을 만든다.
    어휘: 기준, 절차, 발생, 조정, 검토, 보고, 자료 등 실무 어휘 4개 이상 포함.
    문장 구조: 간단한 복문, 요약적 서술, 관찰적 표현을 포함해 1단계보다 난도를 확실히 높인다.
    과장된 묘사나 감정적 표현은 최소화한다.
    분량: 480~520자.
    ${COMMON_RULES}
      `,
    
      3: `
    20대 성인의 사고 확장을 목표로 한 분석형 글을 작성한다.
    주제는 사회, 관계, 심리, 조직문화, 도시 생활 등 구조적 요소가 있는 소재 중 1개를 선택한다.
    구성: 문제 제기 → 배경 설명 → 원인·요인 분석 → 간결한 함의로 이어지는 분석 글.
    어휘: 의의, 관점, 구조, 요인, 맥락, 변동, 조정, 긴장 등 개념어 5개 이상 포함.
    문장 수준: 중급 이상의 복문, 인과 연결, 비교 서술, 추론적 문장 등을 적극 사용한다.
    구체 사례 1개는 포함하되 과도하게 서사적이면 안 된다.
    분량: 480~520자.
    ${COMMON_RULES}
      `,
    
      4: `
    20대 사회초년생을 대상으로 한 고급 비판·통찰형 글을 작성한다.
    주제는 사회적·문화적·정책적·철학적 논점 중 하나로 하되, 일상적 경험이나 미시적 사례와 1회 연결하여 사고를 확장한다.
    구성: 주장 제시 → 근거 전개 → 구조적 분석 → 압축적 함의.
    문체: 정책브리프·학술 칼럼 톤을 참고해 단단하고 밀도 높은 문장을 사용한다.
    어휘: 합의, 담론, 정당성, 구조적 요인, 상충, 지속 가능성, 맥락적 함의, 제도적 한계 등 고급 어휘 5개 이상 포함.
    문장 난도: 다층적 복문·추상적 개념·함축적 표현 포함. 과도한 장황함은 금지하되 밀도는 높인다.
    분량: 480~520자.
    ${COMMON_RULES}
      `,
    };
...
function buildPrompt({ categoryKor, len = 80 }) {
  ...
  - 각 항목은 { "type": "...", "question": "...", "options": [...], "answer": "...", "explanation": "..." } 형식
  - 단답형(SHORT): 반드시 한 문장 내에서 밑줄 친 단어(예: __오늘__)를 문맥에 맞게 다시 써야 하는 문제로 출제하되, 문제에 정답이 노출되어서는 안됨. 정답은 1~2개 단어로 명확하게. 
    예시: "다음 문장에서 밑줄 친 단어를 상황에 맞게 다시 쓰세요. '나는 __오늘__ 보고서를 제출했다.' 정답: 금일"

  수준별 기준:
  1) [취업준비] 프롬프트  
   취업을 준비하는 20대 사회초년생이 실제 구직 과정에서 마주칠 수 있는 ‘비즈니스 표현과 맥락 이해’를 점검할 수 있는 문해력 문제를 작성하세요.  
   조건: 자기소개서, 면접, 사내 커뮤니케이션, 이메일 등 실무 직전 단계에서 자주 쓰이는 표현 중심  
        헷갈리기 쉬운 공적 표현(예: ‘지원하다’ vs ‘지망하다’, ‘협조’ vs ‘협력’)을 보기로 구성  
        문맥 이해와 어휘 선택 능력을 함께 평가하도록 할 것  

  -------------------------------------------------
  2) [기초] 프롬프트  
   사회초년생이 일상과 직장 초입에서 기본적으로 알아야 할 기초 어휘와 표현을 이해하는지를 점검할 수 있는 문제를 작성하세요.  
   조건: 일상 대화, 기본 업무 상황 등 단순 문맥 속에서 출제  
        보기는 비슷한 뜻의 일상 단어들로 구성하되, 문맥상 적절한 단어를 고르는 형태로 구성  
        읽고 상황을 판단해 올바른 어휘를 고르는 문제 포함  

  -------------------------------------------------
  3) [활용] 프롬프트  
   사회초년생이 실제 직장 내 문서·회의·보고 등에서 어휘를 적절히 활용할 수 있는지를 점검할 수 있는 문제를 작성하세요.  
   조건: 공지사항, 보고서, 뉴스 기사 등 실제 문서형 맥락 반영  
        의미가 유사하지만 사용 맥락이 다른 단어(예: ‘제출하다’ vs ‘전달하다’)를 보기로 구성  
        문맥 이해를 통해 단어의 뉘앙스를 구분하도록 유도  

  -------------------------------------------------
  4) [심화] 프롬프트  
   사회초년생의 논리적 사고력과 문해력을 동시에 측정할 수 있는 문제를 작성하세요.  
   조건: 분석, 평가, 입장 제시 등 논리적 문장 구조 포함  
        심화 어휘(예: ‘한계’, ‘전망’, ‘요인’, ‘갈등’)를 활용한 지문 구성  
        보기는 의미는 유사하되 쓰임이나 논리 전개상 맞지 않는 단어를 섞어 구성  
        문장 간 관계를 추론하거나 글의 논리 전환을 이해해야 풀 수 있도록 할 것  

  -------------------------------------------------
  5) [고급] 프롬프트  
   사회초년생이 사회·인문학적 주제 속에서 사고하고, 고급 어휘를 비판적으로 이해하는 능력을 평가할 수 있는 문제를 작성하세요.  
   조건: 사회적 현상, 인문적 주제, 조직문화 등 고급 맥락 반영  
        어휘 예시: ‘담론’, ‘합의’, ‘구조’, ‘성찰’, ‘개입’ 등  
        보기는 모두 근사해 보이지만, 문맥상 가장 적절한 단어를 구분하도록 구성  
        단순 의미 파악이 아닌 ‘맥락 속 사용 적합성’을 중심으로 출제  

  -------------------------------------------------
  설명(explanation) 규칙:
    - '정답:'으로 시작 금지, 정답 텍스트만 반복 금지
    - MCQ: 정답이 문맥상 왜 가장 적절한지 1~2문장으로 간단히 설명 (다른 보기와의 차이 포함)
    - OX: 사실 여부의 근거를 제시 (지문 내용이나 일반적 상식 근거)
    - SHORT: 바꿔 쓴 단어가 문맥상 왜 더 적절한지 1문장으로 설명
    - 모든 설명은 학습자의 이해를 돕는 해설로, 어휘 의미를 ‘문맥 속 사용 예시’로 들어줄 것  

  추가 조건:
  1. 문제 유형은 [사지선다형] / [O,X형] / [단답형]을 섞어 구성할 것.
    - 사지선다형: 보기는 모두 자연스러워야 하며, 정답은 1개만 명확히 존재할 것.
    - OX형: 직장·사회생활 맥락에 맞는 짧은 지문 제시 후, 사실 여부 판단.
    - 단답형: 단순 정의형이 아닌, 문맥상 어휘 교체·적용 중심으로 구성.
      정답은 반드시 1~2개 단어로 명확하며, 주관적 해석 여지를 없앨 것.
