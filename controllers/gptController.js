...
//     `;
    ...
    const todayCategoryKey = categoryByDay[day];
    const todayTopics = topicsByCategory[todayCategoryKey];

    //  오늘 주제 1개 확정
    const chosenTopic =
    todayTopics[Math.floor(Math.random() * todayTopics.length)];
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const combinedPrompt = [
        basePrompt,
        `오늘 날짜: ${today}`,
        `이번 글의 주제는 "${chosenTopic}"이다.`,
        `주제는 이 하나뿐이며, 다른 개념이나 인접 주제로 확장하지 마라.`,
