...
주제는 직장·일상·친구·습관 등 익숙한 소재 중 하나로 하세요.
문장은 짧고 단순하게 구성하고, '금일, 작성, 참조, 절차' 같은 쉬운 행정·생활 용어를 자연스럽게 2개 이상 포함하세요.
스타일: 친절하고 담백한 서술체.
구성: 하나의 사건/감정/관찰을 중심으로 명확한 흐름을 만들 것.
분량: 480~520자.
${COMMON_RULES}`,

      2: `20대 사회초년생을 위해 실무·사회생활·뉴스 배경을 반영한 문해력 강화 글을 작성하세요.
문체는 보고서, 기관 안내문, 절차 설명문의 톤을 참고해 단정하고 명료하게 작성하세요.
어휘: 기준, 절차, 발생, 조정, 검토, 보고 등 실무 어휘를 3개 이상 포함.
구성: 현황 → 설명 → 시사점의 간단한 흐름.
분량: 480~520자.
${COMMON_RULES}`,

      3: `20대 성인의 사고 확장을 돕는 분석형 글을 작성하세요.
주제는 사회·심리·관계·문화 중 하나를 선택해 논리적 연결(원인→결과, 비교, 사례)을 포함하세요.
문체는 연구소·정책센터의 대중 칼럼 톤을 참고해 분석적이되 과밀하지 않게 구성하세요.
어휘: 의의, 관점, 구조, 요인, 맥락, 갈등, 변동 같은 개념어 4개 이상 포함.
구성: 문제 제기 → 배경 설명 → 분석 → 함의.
분량: 480~520자.
${COMMON_RULES}`,

      4: `20대 사회초년생의 비판적 사고력을 높이는 고급 글을 작성하세요.
주제는 사회적·정책적·철학적 논점을 다루되, 일상에서 연결되는 지점을 1개 이상 포함하세요.
문체는 정책브리프·연구보고서 칼럼 톤을 참고해 압축적이고 응축된 문장 사용.
어휘: 합의, 담론, 정당성, 구조적 요인, 상충, 지속 가능성 등 고급 어휘 4개 이상 포함.
구성: 주장 제시 → 근거 설명 → 구조적 분석 → 짧은 함의.
분량: 480~520자.
${COMMON_RULES}`,
    };
//     const levelPrompts = {
